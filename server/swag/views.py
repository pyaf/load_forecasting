...
def forecasted_plot(request):
    ...
    if request.method == 'POST':
        ...
        # for i in range(0,30):
        #     print(i ,"ayushJXHDKJHFKJDBH")
        #     query_results1.append(CSV.objects.filter(date = aa-timedelta(days=(30-i))))
        #     # q = [str(aa-timedelta(days=i))]
        #     loaddata = []
        #     for x in query_results1[i]:
        #         loaddata.append(x.load_value)
        #     last1.append(loaddata)
        # alpha = [0]*288
        # alphamin = [0]*288
        # forecast = [[0]*288 for i in range(30)]
        # forecast[1] = [last1[0][k] for k in range(288)]
        # for j in range(288):
        #     mse = [0]*9
        #     for k in range(1,10):
        #         alpha[j] = k*0.1
        #         for i in range(2,30):
        #             forecast[i][j] = (alpha[j]*last1[i-1][j]) + ((1 - alpha[j])*forecast[i-1][j])
        #         for i in range(1,30):
        #             mse[k-1] += (forecast[i][j] - last1[i][j])**2
        #     minimum = mse[0]
        #     alphamin[j] = 0.1
        #     for i in range(1,9):
        #         if mse[i]<minimum:
        #             minimum = mse[i]
        #             alphamin[j] = (i+1)*0.1
        # forecasted = [[] for i in range(21)]
        # forecasted[1] = last1[10]
        # for j in range(2,21):
        #     for i in range(288):
        #         forecasted[j].append((alphamin[i]*last1[j+10-1][i]) + ((1 - alphamin[i])*forecasted[j-1][i]))
        # # print(".........",forecasted[20])
        # forecasted[20].insert(0,"forecasted_with_SES")
