...
class HoltWintersEstimator(Estimator):
    ...
    def seasonalFactor(self, time):
        ...
        tendence = (
            ((self.season_size + 1.0) / 2.0) - seasonal_index
        ) * tendence_component
    ...
    def estimate(self, time, base_time):
        ...
        estimation = (
            self.average_component[base_time]
            + self.tendence_component[base_time] * (time - base_time)
        ) * self.getSeasonalComponent(time - self.season_size)
        return estimation
    ...
    def initializeComponents(self):
        ...
        # tendence component
        self.tendence_component.append(
            (last_season_average - first_season_average)
            / ((self.number_of_seasons - 1) * self.season_size)
        )

        # average component
        self.average_component.append(
            first_season_average
            - ((self.season_size / 2.0) * self.tendence_component[0])
        )
        ...
        for seasonal_index in range(1, self.season_size + 1):
            ...
            seasonal_index_average.append(
                seasonal_index_sum * (1.0 / self.number_of_seasons)
            )
    ...
    def calculateComponents(self, alpha, beta, gamma):
        for time in range(1, len(self.time_series) + 1):
            average_component = (
                alpha
                * (
                    self.time_series[time - 1]
                    / self.getSeasonalComponent(time - self.season_size)
                )
            ) + (
                (1 - alpha)
                * (self.average_component[time - 1] + self.tendence_component[time - 1])
            )
            self.average_component.append(average_component)

            tendence_component = (
                beta * (self.average_component[time] - self.average_component[time - 1])
            ) + ((1 - beta) * self.tendence_component[time - 1])
            self.tendence_component.append(tendence_component)

            seasonal_component = gamma * self.time_series[
                time - 1
            ] / self.average_component[time] + (1 - gamma) * self.getSeasonalComponent(
                time - self.season_size
            )
