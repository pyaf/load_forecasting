...
def get_data():
    return pd.read_csv('monthdata.csv', header=None, index_col=['datetime'], names=['datetime', 'load'], parse_dates=['datetime'], infer_datetime_format=True)
...
if os.path.exists('monthdata.csv'):
    data = get_data()
    if (datetime.today() - timedelta(1)).date() not in data.index.date:  # yesterdays data not present, scrap it
        ...
        data.to_csv('monthdata.csv', header=False)  # IMP: don't add any header to the monthdata.csv
    else:
...
pred = model.get_prediction(start=pd.to_datetime('%s 00:00:00' % todays_date), end=pd.to_datetime('%s 00:00:00' % tommorows_date), dynamic=False)
